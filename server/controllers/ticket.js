...
export async function createTicket(req, res) {
    try {
        ...
        await createNotification(
            ...
            `Your ${
                consultationType?.replace("_", " ") || "photo review"
            } consultation has been submitted successfully. Ticket ID: ${
                ticket._id
            }`,
            {
                ticket: ticket._id,
        ...
        if (preferredDermatologist) {
            ...
            if (
                preferredDerm &&
                preferredDerm.role === "dermatologist" &&
                preferredDerm.isActive
            ) {
        ...
        if (dermatologists.length === 0) {
            dermatologists = await User.find({
                role: "dermatologist",
                isActive: true,
            });
        }
        ...
        for (const derm of dermatologists) {
            await createNotification(
                ...
                `A new ${
                    consultationType?.replace("_", " ") || "photo review"
                } consultation has been submitted by ${ticket.user.name}`,
                {
                    ticket: ticket._id,
                    sender: req.user.id,
                    actionRequired: true,
...
export async function listTickets(req, res) {
    try {
        ...
        if (req.user.role === "dermatologist") {
            query = {
                $or: [{ dermatologist: req.user.id }, { status: "pending" }],
            };
        } else {
...
export async function provideConsultation(req, res) {
    try {
        ...
        ticket.status = "consultation-provided";
        ...
        await createNotification(
            ...
            "consultation_ready",
            "Consultation Ready",
            `Your consultation is complete! Dr. ${ticket.dermatologist.name} has provided diagnosis and recommendations. Please proceed with payment to access your full consultation report.`,
            {
                ticket: ticket._id,
...
export async function markAsSolved(req, res) {
    try {
        ...
        // Notify user that consultation is fully resolved
        await createNotification(
            ticket.user._id,
            "consultation_resolved",
            "Consultation Resolved",
            `Your consultation with Dr. ${req.user.name} has been marked as resolved.`,
            {
                ticket: ticket._id,
                sender: req.user.id,
                actionUrl: `/tickets/${ticket._id}`,
...
export async function createPaymentSession(req, res) {
    try {
        ...
        if (ticket.status !== "consultation-provided") {
            return res
                .status(400)
    ...
    try {
        ...
        if (ticket.skinType || ticket.symptoms?.length || ticket.duration) {
            ...
            if (ticket.previousTreatments)
                doc.text(`Previous Treatments: ${ticket.previousTreatments}`);
            if (ticket.allergies)
...
export async function downloadConsultationPDF(req, res) {
    try {
        ...
        // PDF Header
        doc.fontSize(20).text("Medical Consultation Report", {
            align: "center",
        });
        doc.moveDown();
        ...
        doc.fontSize(14).text("Professional Consultation:", {
            underline: true,
        });
        doc.fontSize(12);
        doc.text(
            `Dermatologist: Dr. ${ticket.dermatologist?.name || "Unknown"}`
        );
