...
export default function ConsultationRequest() {
  ...
  const consultationTypes = [
    { value: 'photo_review', label: 'Photo Review ($50)', description: 'Upload photos for dermatologist review' },
    { value: 'video_call', label: 'Video Consultation ($100)', description: 'Live video call with dermatologist' },
    { value: 'follow_up', label: 'Follow-up ($30)', description: 'Follow-up consultation' }
  ];
  ...
  const loadAvailableSlots = async () => {
    try {
      const response = await API.get(`/tickets/dermatologist/${selectedDermatologist}/slots`);
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      const uploadPromises = files.map(async (file) => {
        ...
        const response = await API.post('/upload/consultation', formData, {
      ...
      setPhotoPreviews(prev => [...prev, ...uploadedPhotos.map(p => p.preview)]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (photos.length === 0 && consultationType === 'photo_review') {
      alert('Please upload at least one photo for photo review consultation');
      return;
    }
    ...
    try {
      ...
      if (consultationType === 'video_call' && selectedDermatologist && selectedSlot) {
        const slot = availableSlots.find(s => s.id === selectedSlot);
        const scheduledDateTime = new Date(`${slot.date}T${slot.time}:00`);
        ...
        alert('Video consultation booked successfully! You will receive confirmation from the dermatologist.');
      ...
      const submissionData = {
        ...formData,
        photos: photos.map(p => ({ url: p.url, description: p.description })),
        consultationType,
      ...
      alert('Consultation request submitted successfully! You will be notified when a dermatologist reviews your case.');
      ...
      <form onSubmit={handleSubmit} style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {consultationTypes.map(type => (
              <label key={type.value} style={{
                ...
                padding: '16px',
                border: '2px solid',
                borderColor: consultationType === type.value ? '#3b82f6' : '#e5e7eb',
                borderRadius: '8px',
                cursor: 'pointer',
                background: consultationType === type.value ? '#f0f9ff' : 'white'
              }}>
                <input
                  type="radio"
                ...
                />
                <div>
                  <div style={{ fontWeight: '600', color: '#1f2937' }}>{type.label}</div>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <select
            ...
            onChange={(e) => setSelectedDermatologist(e.target.value)}
            style={{
              width: '100%',
              padding: '12px',
          ...
          >
            ...
            {dermatologists.map(derm => (
              <option key={derm._id} value={derm._id}>
                Dr. {derm.name} {derm.profile?.qualification && `- ${derm.profile.qualification}`}
              </option>
            ))}
          </select>
        </div>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            {availableSlots.length > 0 ? (
              <div style={{
                ...
                {availableSlots.map(slot => (
                  <label key={slot.id} style={{
                    ...
                    padding: '12px',
                    border: '2px solid',
                    borderColor: selectedSlot === slot.id ? '#3b82f6' : '#e5e7eb',
                    borderRadius: '8px',
                    cursor: 'pointer',
                    background: selectedSlot === slot.id ? '#f0f9ff' : 'white'
                  }}>
                    <input
                      type="radio"
                      name="timeSlot"
              ...
              <p style={{ color: '#6b7280', fontStyle: 'italic' }}>
                {selectedDermatologist ? 'No available slots for selected dermatologist' : 'Please select a dermatologist to view available slots'}
              </p>
            )}
          </div>
        )}
          ...
          <div style={{
            ...
            {symptomOptions.map(symptom => (
              <label key={symptom} style={{
                ...
                alignItems: 'center',
                padding: '8px',
                background: formData.symptoms.includes(symptom) ? '#f0f9ff' : '#f9fafb',
                border: '1px solid',
                borderColor: formData.symptoms.includes(symptom) ? '#3b82f6' : '#e5e7eb',
                borderRadius: '6px',
                cursor: 'pointer',
                fontSize: '14px'
              }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          >
            ...
            <option value="medium">Medium - Would like response within 24 hours</option>
            <option value="high">High - Need response as soon as possible</option>
          </select>
        </div>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{
              ...
              <label htmlFor="photo-upload" style={{
                ...
                padding: '12px 24px',
                background: uploading || photos.length >= 5 ? '#d1d5db' : '#3b82f6',
                color: 'white',
                borderRadius: '8px',
              ...
              <p style={{ marginTop: '8px', fontSize: '12px', color: '#6b7280' }}>
                Upload clear photos of the affected area. {photos.length}/5 photos uploaded.
              </p>
            </div>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '14px', color: '#1e40af', margin: 0 }}>
            Payment will be required after consultation is completed.
            {consultationType === 'video_call' && ' Video consultations include a live 30-minute session.'}
          </p>
        </div>
        ...
        <button
          type="submit"
          disabled={submitting || uploading || (consultationType === 'photo_review' && photos.length === 0)}
          style={{
            width: '100%',
            padding: '16px',
            background: submitting || uploading || (consultationType === 'photo_review' && photos.length === 0) 
              ? '#d1d5db' : '#10b981',
            color: 'white',
            border: 'none',
            borderRadius: '8px',
            fontSize: '16px',
            fontWeight: '600',
            cursor: submitting || uploading || (consultationType === 'photo_review' && photos.length === 0) 
              ? 'not-allowed' : 'pointer'
          }}
        >
          {submitting ? 'Submitting Request...' : `Book ${consultationType === 'video_call' ? 'Video ' : ''}Consultation`}
        </button>
      </form>
    </div>
  );
